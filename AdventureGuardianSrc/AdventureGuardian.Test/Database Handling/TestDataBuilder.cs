...
public class TestDataBuilder
{
    ...
    public TestDataBuilder(AdventureGuardianDbContext dbContext)
    {
        ...
        publisherMock.Setup(m => m.SendMessage(It.IsAny<RequestEvent>(), It.IsAny<OpenAiRequestRoute>()));
        
        CharacterService = new CharacterService(new CharacterRepository(dbContext), publisherMock.Object);
        CampaignService = new CampaignService(new CampaignRepository(dbContext, new ClaimsHandlerService(httpContextAccessorMock.Object)),
            new ClaimsHandlerService(httpContextAccessorMock.Object), new WorldService(publisherMock.Object));
